...
type httpClient struct {
	base    *http.Client
	baseURL string
	backend string // backend name for observability (e.g., "quantinuum.H1-1")
	auth    *tokenProvider
}
...
func (c *httpClient) doOnce(ctx context.Context, method, path string, body, resp any) error {
	...
	if body != nil {
		...
		if err != nil {
			...
			return fmt.Errorf("quantinuum: marshal request: %w", err)
		}
	...
	if err != nil {
		...
		return fmt.Errorf("quantinuum: create request: %w", err)
	}
	...
	if err != nil {
		...
		return fmt.Errorf("quantinuum: http request: %w", err)
	}
	...
	if err != nil {
		...
		return fmt.Errorf("quantinuum: read response: %w", err)
	}
	...
	if resp != nil && len(respBody) > 0 {
		if err := json.Unmarshal(respBody, resp); err != nil {
			...
			return fmt.Errorf("quantinuum: unmarshal response: %w", err)
		}
	}
...
// APIError represents an error response from the Quantinuum API.
...
func (e *APIError) Error() string {
	if e.Code != "" {
		return fmt.Sprintf("quantinuum: %d %s: %s", e.StatusCode, e.Code, e.Message)
	}
	return fmt.Sprintf("quantinuum: %d: %s", e.StatusCode, e.Message)
}
